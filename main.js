...
function showData(products) {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    updateElement.innerHTML = `<button class="btn bg-green update" data-index="${i}"> Update </button>`;
    row.appendChild(updateElement);

    let dele = document.createElement("td");
    dele.innerHTML = `<button class="btn bg-red delete" data-index="${i}"> Delete </button>`;
